...
// filters the table on click
    ...
    eventHandler.getDateRange = function() {
        var dateStart = moment($('#start-date').val()).startOf('day').isValid() ? moment($('#start-date').val()).utc().startOf('day') : moment($('#download-start-date').val()).utc().startOf('day');
        dateStart = dateStart.isValid() ? dateStart : moment('2017-01-01').utc().startOf('day');
        var dateEnd = moment($('#end-date').val()).utc().endOf('day').isValid() ? moment($('#end-date').val()).utc().endOf('day') : moment($('#download-end-date').val()).utc().endOf('day');
        dateEnd = dateEnd.isValid() ? dateEnd : moment().endOf('day');
        var start = dateStart.unix();
        var end = dateEnd.unix();
        var dates = [];
