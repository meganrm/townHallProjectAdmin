...
// slightly hacky routing
  ...
  ///Archive submissions
    eventHandler.archiveSubmission = function(event){
      ...
      var state = event.target.dataset.state ? event.target.dataset.state : 'federal'; ///To prevent permissions errors when checking if state path exists
      var statePath = `/state_townhalls/${state}/`;
      var path = '';
      var archivePath = '';

      ///Set path
      if (event.target.dataset.path) {path = event.target.dataset.path;}
      else if(event.target.dataset.state) {path = statePath;}
      else {path = '/townHalls/';}
      

    ///Check if state path exists
      return firebasedb.ref(statePath).once('value').then(function(snapshot){
        if(!snapshot.exists()){
          archivePath = '/townHallsOld/';
      } else {
          archivePath = `/state_townhalls_archive/${state}/`;
      }
    }).then(
      function(){
          return new Promise(function (resolve, reject) {
            ...
            .then(function (snapshot) {
                ...
                if (snapshot.exists()) {
                  ...
                  firebasedb.ref(archivePath + dateKey + '/' + id).update(townHall);
