(function(module) {
    ...
    helperFunctions.updateOldData = function updateOldData(oldValue, newValue, dateParam) {
        var date_key;
        var date_list;
        if (dateParam) {
            date_list = [dateParam];
        } else {
            date_list = ['2016-4', '2017-0', '2017-1', '2017-2', '2017-3', '2017-4', '2017-5', '2017-6', '2017-7', 'noDate'];
        }
                    ...
                    switch(newValue) {
                        ...
                        default:
                            if(townHallObj[oldValue]) {
                                ...
                                updateObj(`/townHallsOld/${date_key}/${oldTownHall.key}`, newValue, currentValue);
                            } else {
    ...
    function districtUpdate (townHallObj, key, date_key) {
        ...
        if (townHallObj[oldValue]) {
            ...
            if (isNaN(districtString)) {
                ...
                updatedDistrict = zeropadding.slice(0, zeropadding.length - districtString.length) + districtString;                    
            }
    ...
    function stateNameUpdate (townHallObj, key, date_key) { // 9 values return no property found (info is logged)
        var oldValue = 'State';
        ...
        if (townHallObj[oldValue]) {                
            ...
            if (currentValue.length === 2) {
                ...
                var stateName = statesAb[currentValue]; // what is happening to this value?? stateKeys??
            } else {
        ...
        if(!townHallObj[oldValue]) {
            console.log("No " + oldValue + " property found. " + date_key + " " + key);
        }
    }
    ...
    function stateUpdate (townHallObj, key, date_key) {
        ...
        if (townHallObj[oldValue] && townHallObj[oldValue].length == 2 && typeof townHallObj[oldValue] !== 'undefined') { 
    ...
    function addGovTrackId (townHallObj, key, date_key) {
        if (typeof townHallObj.Member !== 'undefined') {
            getMember(townHallObj.Member).then(function(govtrack_id) {
                ...
                updateObj(`/townHallsOld/${date_key}/${key}`, govtrack_id, govtrack_id);
    ...
    getMember = function (displayName) {
        ...
        switch(memberKey) {
            ...
            case 'sanchez_linda':
                ...
                break;
            case 'conyers,jr._john':
                memberKey = 'conyers_john';
                break;
        ...
        return new Promise(function(resolve, reject) {
            firebase.database().ref('mocID/' + memberKey).once('value').then(function (snapshot) {
                if (snapshot.exists()) {
                    ...
                    reject(memberKey + ' That member is not in our database, please check the spelling, and only use first and last name.')
                }
            })
        })
    }
